...
class VelocitySmoothing(object):
    ...
    def updateDurations(self, t):
        ...
        if abs(self._accel) > FLT_EPSILON and do_stop_check:
            ...
            vel_zero_acc = (
                self._vel
                + self._accel * t_zero_acc
                + 0.5 * j_zero_acc * t_zero_acc * t_zero_acc
            )
            verboseprint(
                "vel_zero_acc = {}\tt_zero_acc = {}".format(vel_zero_acc, t_zero_acc)
            )
            verboseprint(
                "vel = {}, accel = {}, jerk = {}".format(
                    self._vel, self._accel, j_zero_acc
                )
            )
        else:
    ...
    def evaluateTraj(self, t_now):
        ...
        if t <= self._T1:
            t1 = t
            (self._jerk, self._accel, self._vel, self._pos) = self.evaluatePoly(
                j, self._at0, self._vt0, self._xt0, t1, d
            )
            ...
            (self._jerk, self._accel, self._vel, self._pos) = self.evaluatePoly(
                0.0, self._accel, self._vel, self._pos, t2, 0.0
            )
            ...
            (self._jerk, self._accel, self._vel, self._pos) = self.evaluatePoly(
                j, self._accel, self._vel, self._pos, t3, -d
            )
