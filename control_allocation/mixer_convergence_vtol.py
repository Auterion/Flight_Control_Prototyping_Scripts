...
def plot_arrow(start, end, figure_handle, axis_handle=None):
    ...
    vec = Arrow3D(
        [start[0], end[0]], [start[1], end[1]], [start[2], end[2]], arrowstyle=style
    )
...
torque_thurst_demanded = np.matrix(
    np.concatenate((torque_demanded, thrust_demanded), axis=None)
)
...
u_fx_orig, u_fx_opt = mix_forward_thrust_and_yaw(
    input_vector, P_fx_yaw, thrust_x_min.T, thrust_x_max.T
)
